...
plt.bar(list(range(len(indices))), importances[indices], color='lightblue', align="center")
plt.step(list(range(len(indices))), np.cumsum(importances[indices]), where='mid', label='Cumulative')
plt.xticks(list(range(len(indices))), feat_names[indices], rotation='vertical',fontsize=14)
