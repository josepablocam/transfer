...
# Time for some general data inspection. Let's first examine to see if there are any nulls in the dataframe as well as look at the type of the data (i.e whether it is a string or numeric)
...
for colname, colvalue in house.items():
...
sns.factorplot(x="Mean Ranking", y="Feature", data = meanplot, kind="bar", size=4, aspect=1.9, palette='PuBuGn_d')


# Well as you can see from our feature ranking endeavours, the top 3 features are 'lat', 'waterfront' and 'grade', while the bottom 3 are 'sqft_lot15', 'sqft_lot' and 'sqft_basement'. 
