...
def build_docker_command(docker_image, timeout, script_path, vm_output_dir,
    ...
    docker_command = [
        'docker', 'run', '--name', container_name, '-v', '{}:{}'.format(
            vm_output_dir, docker_output_dir), '--memory', mem_limit
    ]
    if not keep:
        docker_command += ['--rm']
...
def create_job_commands(docker_image, scripts_dir, vm_output_dir,
                        docker_output_dir, timeout, mem_limit, keep,
                        regex_pattern, plain):
