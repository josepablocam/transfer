...
def build_docker_command(
    docker_image, timeout, script_path, vm_output_dir, docker_output_dir,
    mem_limit, keep, plain
):
    ...
    docker_command = [
        ...
        '{}:{}'.format(vm_output_dir, docker_output_dir), '--memory', mem_limit
    ]
    if not keep:
        docker_command += ['--rm']
...
def schedule_jobs(
    docker_image, scripts_dir, vm_output_dir, docker_output_dir, timeout,
    ...
    for script_name in scripts:
        ...
        cmd = build_docker_command(
            docker_image, timeout, script_path, vm_output_dir,
...
def main(args):
    schedule_jobs(
        args.docker_image, args.scripts_dir, args.vm_output_dir,
        args.docker_output_dir, args.timeout, args.mem_limit, args.keep,
        args.regex, args.plain
    )
...
if __name__ == '__main__':
    ...
    parser.add_argument(
        ...
        '--regex',
        type=str,
        help='Only schedule scripts with a path that matches the regex',
        default=None
    )
    parser.add_argument(
        '-p',
