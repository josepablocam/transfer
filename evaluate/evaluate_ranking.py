...
def load_distance_computer(db, distance_computer, distance_type):
    if distance_computer is None:
        ...
        if os.path.exists(distance_computer):
            print(
                'Attempting read pre-computed distances from: {}'.
                format(distance_computer)
            )
...
def compute_test_data(db, test_nodes, distance_computer):
    ...
    for test_node in tqdm.tqdm(test_nodes):
        rels_for_query = db.get_extracted_function_relationships_from_node(
            test_node
        )
        ...
        distances = distances_from_nodes(
            test_fun, db, query_results, distance_computer
        )
        # use function names to break ties for deterministic sorting...
        ranking_metric = list(
            zip(distances, [n['name'] for n in query_results])
        )
...
def run(
    ...
    distance_computer = load_distance_computer(
        db, distance_computer, distance_type
    )
    data = []
        ...
        for i in tqdm.tqdm(list(range(n_iters))):
            ...
            for model_constructor in tqdm.tqdm(model_constuctors):
                ...
                for test_node in tqdm.tqdm(test_nodes):
                    ...
                    optimally_ranked_results = test_data[test_node][
                        'query_results']
                    ...
                    if len(optimally_ranked_results) > 0:
                        ...
                        test_ix = ixs[(ref_query_nodes == test_node)
                                      & ~is_test_result]
                        orig_ordered_result_nodes = ref_result_nodes[test_ix]
                        test_ix_dict = dict(
                            zip(orig_ordered_result_nodes, test_ix)
                        )
                        test_ix = [
                            test_ix_dict[r] for r in optimally_ranked_results
                        ]
                        ...
                        result_nodes_names = [
                            n['name'] for n in ordered_result_nodes
                        ]
                        pred_ranking_metric = list(
                            zip(model.predict(X_test), result_nodes_names)
                        )
                        pred_ranked_results = sort_by_values(
                            optimally_ranked_results, pred_ranking_metric
                        )
                        assert len(pred_ranked_results
                                   ) == len(optimally_ranked_results)

                        # evaluate ranking relative to ideal
                        optimally_ranked_results_names = [
                            n['name'] for n in optimally_ranked_results
                        ]
                        pred_ranked_results_names = [
                            n['name'] for n in pred_ranked_results
                        ]
                        info['spearman_corr'], info[
                            'spearman_pval'] = spearman_correlation(
                                optimally_ranked_results_names,
                                pred_ranked_results_names
                            )
                        ...
                        test_fun = db.get_function_from_node(test_node)
                        info['distances_for_preds'] = distances_from_nodes(
                            test_fun, db, pred_ranked_results,
                            distance_computer
                        )
                        info['distances_for_optimal'] = distances_from_nodes(
                            test_fun, db, optimally_ranked_results,
                            distance_computer
                        )
                        info['query_results_names'
                             ] = optimally_ranked_results_names
                    data.append(info)
...
if __name__ == '__main__':
    parser = ArgumentParser(
        description='Evaluate database query result ranking by LOOCV'
    )
    parser.add_argument(
        'database_file', type=str, help='Pickled database interface file'
    )
    parser.add_argument(
        'n_iters', type=int, help='Number of iterations for each test_node'
    )
    parser.add_argument(
        'output_path',
        type=str,
        help='Path to store pickled dataframe with evaluation raw data'
    )
    parser.add_argument(
        '-t',
        '--distance_type',
        type=str,
        help='Type of distance computation',
        default='string'
    )
    parser.add_argument(
        '-d',
        ...
        type=str,
        help=
        'Path to existing (or desired location for storage) for distance computer'
    )
